...
function scene:create( event )
    ...
    titleText = display.newText("THUMB RACER", display.contentWidth/2, -22, "Arial", 35 )
    titleText:setFillColor( 0,0,1 )
    sceneGroup:insert(titleText)

    centerLine = display.newLine( display.contentWidth/2, 0, display.contentWidth/2, display.contentHeight+50 )
    centerLine:setStrokeColor( 0, 0, 1, 1 )
    centerLine.strokeWidth = 8
    sceneGroup:insert(centerLine)

    topLine = display.newLine(0,0, display.contentWidth, 0)
    topLine:setStrokeColor( 0, 0, 1, 1 )
    topLine.strokeWidth = 8
    sceneGroup:insert(topLine)

    footPlacement = display.newRect( 70, display.contentHeight, 170, 200 )
    footPlacement.strokeWidth = 1
    footPlacement:setFillColor( 0,1,0 )
    sceneGroup:insert(footPlacement)
    ...
    highScoreInfo = display.newText( "High Score: ", display.contentWidth/2, display.contentHeight/2 - 60, "Arial", 27 )
    highScoreInfo:setFillColor( 0,0,1 )
    highScoreInfo.isVisible = false
    sceneGroup:insert(highScoreInfo)

    currentScoreInfo = display.newText( "Your Score:", display.contentWidth/2, display.contentHeight/2 - 100, "Arial", 27 )
    currentScoreInfo:setFillColor( 0,0,1 )
    currentScoreInfo.isVisible = false
    sceneGroup:insert(currentScoreInfo)
    ...
    distText = display.newText( distOptions )
    ...
    distValueText = display.newText( distValueOptions )
    distValueText:setFillColor(0,1,0)
    sceneGroup:insert(distValueText)
    ...
    timerText = display.newText( timerTextOptions)
    ...
    buttonRemoveAds = widget.newButton
    {
        label = "Remove Ads",
        labelColor = { default={ 1, 1, 1 }, over={ 0, 0, 0, 0.5 } },
        fontSize = 20,
        onEvent = removeAdsButtonPressed,
        emboss = false,
        --properties for a rounded rectangle button...
        shape="roundedRect",
        width = 200,
    ...
    buttonRemoveAds.x = display.contentCenterX
    buttonRemoveAds.y = display.contentCenterY + 100
    sceneGroup:insert(buttonRemoveAds)
    ...
    sceneGroup:insert(buttonReplay)

    ads = require( "ads" )
    appID=  "ca-app-pub-7134633350641035/6740295304"
    adProvider = "admob"
    ads.init( adProvider, appID, adListener )
    ads:setCurrentProvider( "admob")

    local newAd = ads.show( "banner", { x=0, y=1000000, appID} )
    if(newAd) then
        sceneGroup:insert(newAd)
    end
...
function playButtonPressed( event )
    if ( "ended" == event.phase ) then
        ...
        buttonRemoveAds.isVisible = false
...
function removeAdsButtonPressed( event )
    if ( "ended" == event.phase ) then
        ads.hide()
    	composer.gotoScene( "removeads" )
...
local function timerDown()
   ...
   timeLeft.text = timeLimit
     if(timeLimit==0)then
        ...
        buttonRemoveAds.isVisible = true
